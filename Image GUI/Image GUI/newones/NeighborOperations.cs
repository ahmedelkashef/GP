...
namespace PhotoEditor_2015
{
    class NeighborOperations
    {
        ...
        {
            int Width = Old_Buffer.GetLength(0);
            int Height = Old_Buffer.GetLength(1);
            int PadLeft, PadRight, PadUp, PadDown;
            PadLeft = OriginY;
            PadRight = Width - 1 - OriginY;
            PadUp = OriginX;
            PadDown = Height - 1 - OriginX;

            Color[,] NewBuffer = new Color[Width + PadRight, Height + PadDown];
            int newW = NewBuffer.GetLength(0);
            int newH = NewBuffer.GetLength(1);

            for (int i = 0; i < Width; i++)
            {
                for (int j = 0; j < Height; j++)
                {
                    NewBuffer[i + PadLeft, j + PadUp] = Old_Buffer[i, j];
                }
            }
            return NewBuffer;
        }
